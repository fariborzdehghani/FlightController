 ...
 */
 ...
 static HAL_StatusTypeDef bmp_last_status = HAL_OK;

 static void BMP280_RecordStatus(HAL_StatusTypeDef status)
 {
     if (bmp_last_status == HAL_OK && status != HAL_OK)
 ...
 uint8_t BMP280_Read8(uint8_t addr)
 {
     ...
     uint8_t tmp = 0;
   if (i2c_h == NULL) {
       bmp_last_status = HAL_ERROR;
       return 0;
   }
   BMP280_RecordStatus(HAL_I2C_Mem_Read(i2c_h, BMP280_I2CADDR, addr,
                                       I2C_MEMADD_SIZE_8BIT, &tmp, 1, 10));
   return tmp;
 #endif
 #if(BMP_SPI == 1)
     uint8_t tmp[2];
 ...
 void BMP280_Write8(uint8_t address, uint8_t data)
 {
     ...
     BMP280_RecordStatus(HAL_I2C_Mem_Write(i2c_h, BMP280_I2CADDR, address,
 ...
 #if(BMP_I2C == 1)
 HAL_StatusTypeDef BMP280_Init(BMP280_Config_t config)
 {
     if (config.i2c == NULL)
         return HAL_ERROR;
     i2c_h = config.i2c;
 ...
 void BMP280_Init(BMP280_Config_t config)
 {
     ...
     HAL_GPIO_WritePin(BMP_CS_GPIO_Port, BMP_CS_Pin, GPIO_PIN_SET);
 #endif
    uint8_t temperature_resolution = config.temperature_resolution;
    uint8_t pressure_oversampling = config.pressure_oversampling;
    uint8_t mode = config.mode;
    uint8_t standby_time = config.standby_time;
    uint8_t filter = config.filter;
    ...
    bmp_last_status = HAL_OK;
    const uint32_t chip_id_started = HAL_GetTick();
    uint8_t chip_id = 0u;
    do {
        bmp_last_status = HAL_OK;
        chip_id = BMP280_Read8(BMP280_CHIPID);
        if (bmp_last_status == HAL_OK && chip_id == 0x58u)
            break;
        HAL_Delay(5);
    } while ((HAL_GetTick() - chip_id_started) < 100u);

    if (chip_id != 0x58u)
        return (bmp_last_status != HAL_OK) ? bmp_last_status : HAL_TIMEOUT;
    ...
    /* apply config register values from struct */
    BMP280_SetConfig(standby_time, filter);
    return bmp_last_status;
}
#endif
 #ifdef BME280
 ...
 void BME280_Init(BME280_Config_t config)
 {
    ...
    uint8_t temperature_resolution = config.temperature_resolution;
    uint8_t pressure_oversampling = config.pressure_oversampling;
    uint8_t huminidity_oversampling = config.huminidity_oversampling;
    uint8_t mode = config.mode;
    uint8_t standby_time = config.standby_time;
    uint8_t filter = config.filter;
    uint8_t HumReg, i;
    ...
    if(mode == BME280_NORMALMODE)
    {
        BME280_SetConfig(standby_time, filter);
    }
 }
 #endif
 ...
 float BMP280_ReadTemperature(void)
 {
   ...
   if(_mode == BMP280_FORCEDMODE)
   {
       ...
       const uint32_t conversion_started = HAL_GetTick();
       while(1) // Wait for end of conversion
       {
           ...
           if(mode == BMP280_SLEEPMODE)
               break;
           if ((HAL_GetTick() - conversion_started) >= 100u) {
               bmp_last_status = HAL_TIMEOUT;
               return -99.0f;
           }
       }
   }
   ...
   if (bmp_last_status != HAL_OK || adc_T == 0x800000) {
       if (bmp_last_status == HAL_OK)
           bmp_last_status = HAL_ERROR;
       return -99.0f;
   }
   adc_T >>= 4;
 ...
 int32_t BMP280_ReadPressure(void)
 {
       ...
       const float temperature = BMP280_ReadTemperature();
       if (bmp_last_status != HAL_OK || temperature == -99.0f) {
         if (bmp_last_status == HAL_OK)
           bmp_last_status = HAL_ERROR;
         return 0;
       }
       ...
       if (bmp_last_status != HAL_OK || adc_P == 0x800000) {
         if (bmp_last_status == HAL_OK)
           bmp_last_status = HAL_ERROR;
         return 0;
       }
 ...
 HAL_StatusTypeDef BMP280_ReadAltitude(float *altitude)
 {
       if (altitude == NULL)
           return HAL_ERROR;
       ...
       if (bmp_last_status != HAL_OK || pressure <= 0.0f || !isfinite(pressure))
           return (bmp_last_status != HAL_OK) ? bmp_last_status : HAL_ERROR;
 
       *altitude = 44330.0f * (1.0f - powf(pressure / 101325.0f, 0.1903f));
       return isfinite(*altitude) ? HAL_OK : HAL_ERROR;
 }
 #endif
 #ifdef BME280
